...
4. **YouTube API 설정** (선택사항 - 실시간 인기차트용)

```bash
# .env 파일 생성
cp .env.example .env
# .env 파일에 YouTube Data API v3 키 설정
# YOUTUBE_API_KEY=your_api_key_here
```

YouTube API 키 설정 방법은 `README_API_SETUP.md` 파일을 참조하세요.
...
1. **실시간 인기차트**: 홈 화면에서 현재 인기 있는 음악을 확인하고 바로 재생할 수 있습니다
2. **음악 검색**: 검색 아이콘을 탭하고 듣고 싶은 노래나 아티스트 이름을 입력합니다
3. **음악 재생**: 아무 노래나 탭하여 재생을 시작합니다
4. **플레이리스트 생성**: 즐겨 듣는 노래를 플레이리스트에 저장하여 빠르게 접근할 수 있습니다
5. **백그라운드 재생**: 앱이 백그라운드에 있어도 음악은 계속 재생됩니다
6. **다크 모드**: 설정에서 라이트 모드와 다크 모드를 전환할 수 있습니다
...
- **실시간 인기차트**: YouTube Data API v3를 통한 실제 인기 음악 차트
- 다크 모드 지원
- 백그라운드 재생 기능
- 플레이리스트 생성
- 노래 및 유튜버 검색
...
**Q: 실시간 인기차트는 어떻게 작동하나요?**
A: YouTube Data API v3를 사용하여 실제 YouTube의 한국 지역 음악 인기차트를 10분마다 자동으로 업데이트합니다. API 키가 설정되지 않은 경우 검색 기반 방식으로 자동 전환됩니다.

**Q: YouTube API 키를 설정하지 않으면 어떻게 되나요?**
A: API 키 없이도 앱은 정상적으로 작동합니다. 다만 실시간 인기차트는 검색 기반 방식으로 동작하여 실제 YouTube 인기차트와 다를 수 있습니다.
